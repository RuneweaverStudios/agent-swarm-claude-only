#!/usr/bin/env python3
"""
Agent Swarm (Claude Only) | OpenClaw Skill — Task-to-LLM routing for OpenClaw
Version 1.0.0

Claude-only routing for users with Anthropic API key configured in OpenClaw.
Routes tasks to efficient Claude models (Haiku, Sonnet, Opus) based on complexity.
...
Features:
- Claude model routing (Haiku for FAST, Sonnet for CODE/QUALITY, Opus for complex)
...
class ClaudeRouter:
    """Claude-only intelligent model router."""
    ...
    def classify_task(self, task_description, return_details=False):
        ...
        # Website/frontend projects → CREATIVE, never CODE
    ...
    def get_default_model(self):
        """Return the default model (Claude Haiku). Used for session default and orchestrator."""
    ...
    def recommend_model(self, task_description):
        """Classify task and recommend the best Claude model."""
    ...
    def spawn_agent(self, task, session_target='isolated', label=None, required_exec_host='sandbox', required_exec_node=None):
        """Spawn an OpenClaw sub-agent with the appropriate Claude model.
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Agent Swarm (Claude Only) | OpenClaw Skill - Task-to-LLM routing for OpenClaw.")
    ...
    if args.command == 'default':
        ...
        if args.json:
            ...
            print("\n   Simple tasks down-route to FAST tier (e.g. Claude Haiku).")
        ...
        if args.json:
            ...
            print("📦 Configured Claude Models:\n")
        ...
        if getattr(args, 'multi', False):
            ...
            tasks = ClaudeRouter.split_into_tasks(task_str)
            if not tasks:
                tasks = [task_str]
            results = []
